...
def get_exon_expr(gene, vstart, vstop, countinfo, Idx, seg_counts):
    ...
    seg_counts: np.array, array of spladder segment counts from gene and sample of interest
        (num_segments_in_gene, num_mutation_samples)
...
def get_segment_expr_kmer(gene, kmer_path, countinfo, seg_counts):
    """ Get the segment expression for one k-mer path.

    Parameters
    ----------
    gene: Object. Generated by SplAdder.
    kmer_path: Tuple of tuples. Format: ((seg_id, start, end), (seg_id, start, end), ...)
        Contains the segment coordinates that make up the k-mer path.
    countinfo: Namedtuple, contains count information generated by SplAdder
    seg_counts: np.array, array of spladder segment counts from gene and sample of interest
        Shape: (num_segments_in_gene, num_mutation_samples)

    Returns
    -------
    expr_meta_file: float. Always np.nan (for compatibility with original)
    expr_list: 2D array. Each row contains [segment_length, expr_sample1, expr_sample2, ...]
        representing the length and expression count of each segment portion in the k-mer path.
    """
    expr_meta_file = np.nan

     # Feature flag: if countinfo is None, expression calculation is disabled
    if countinfo is None:
        return expr_meta_file, np.zeros((0, 1), dtype='float')
    
    if not kmer_path:  # empty k-mer path
        out_shape = (seg_counts.shape[1] + 1) if len(seg_counts.shape) > 1 else 2
        return expr_meta_file, np.zeros((0, out_shape), dtype='float')
    
    # Process each segment portion in the k-mer path
    expr_list_segments = []
    
    for seg_id, start, end in kmer_path:
        # Calculate the length of this k-mer portion within the segment
        kmer_portion_length = end - start
        
        # Get expression data for this segment
        if len(seg_counts.shape) > 1:
            # Multi-sample case: seg_counts[seg_id, :] gives expression across all samples
            segment_expression = seg_counts[seg_id, :]
            expr_row = np.array([[kmer_portion_length] + segment_expression.tolist()])
        else:
            # Single sample case
            segment_expression = seg_counts[seg_id]
            expr_row = np.array([[kmer_portion_length, segment_expression]])
        
        expr_list_segments.append(expr_row)
    
    # Concatenate all segment expressions
    if expr_list_segments:
        expr_list = np.vstack(expr_list_segments)
    else:
        # No valid expression data found
        out_shape = (seg_counts.shape[1] + 1) if len(seg_counts.shape) > 1 else 2
